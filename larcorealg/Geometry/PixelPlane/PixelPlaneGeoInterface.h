/**
 * @file    larcorealg/Geometry/PixelPlane/PixelPlaneGeoInterface.h
 * @brief   Representation of a readout plane with pixels as sensitive elements.
 * @author  Gianluca Petrillo (petrillo@slac.stanford.edu)
 * @date    October 23, 2019
 * @see     `larcorealg/Geometry/PixelPlane/PixelPlaneGeoInterface.cxx`
 * @ingroup GeometryPixel
 */

#ifndef LARCOREALG_GEOMETRY_PIXELPLANE_PIXELPLANEGEOINTERFACE_H
#define LARCOREALG_GEOMETRY_PIXELPLANE_PIXELPLANEGEOINTERFACE_H
...
// -----------------------------------------------------------------------------
namespace geo { class PixelPlaneGeoInterface; }

/**
 * @brief Geometry information for a single pixel plane.
 * @ingroup GeometryPixel
 * 
 * This class implements the `geo::PlaneGeo` interface for pixel readout planes,
 * and it initiates the interface for pixel readout planes as well, but, despite
 * the name, it is not only an interface class, and it contains data that needs
 * to be initialized.
 * The initialization is not fully performed in this class, and it is expected
 * that derived classes perform it.
 * 
 * 
 ...
 * `geo::PixelPlaneGeoInterface::SensitivePixelPlaneVolumeName`, this volume
 * is used to determine the sensitive area, as follows:
 * 
 ...
 * * the number and size of pixels determination is customised, with some
 *   provided patterns but none set as default (see `geo::PixelPlaneGeoBase`)
 ...
 * `geo::PixelPlaneGeoInterface::SensitivePixelPlaneVolumeName` in the plane,
 * the plane volume itself is used instead, with the same rules.
 * 
 ...
 * a `geo::PixelPlaneGeoInterface` object.
 * 
 ...
 *    step (see @ref geo_PixelPlaneGeoInterface_Derivation "Usage as derived class"):
 ...
 *    (`geo::PixelPlaneGeoInterface::doUpdateAfterSorting()`, called by the base
 *    class):
 ...
 * -------------------------------
 * 
 * @anchor geo_PixelPlaneGeoInterface_Derivation
 ...
 *    calling `geo::PixelPlaneGeoInterface::doUpdateAfterSorting()`: the
 *    required setup is the one listed below as requirements for
 ...
 * 
 */
class geo::PixelPlaneGeoInterface: public geo::PlaneGeo {
   ...
   * 
   * Technically, the handle refers back to this `geo::PixelPlaneGeoInterface`
   * object.
   */
  ...
  /// --- BEGIN -- Pixel geometry ----------------------------------------------
  
    protected: // still
  
  /// --- BEGIN -- Data for pixel geometry -------------------------------------
...
}; // class geo::PixelPlaneGeoInterface


//------------------------------------------------------------------------------
//--- inline implementation
// -----------------------------------------------------------------------------
inline auto geo::PixelPlaneGeoInterface::wireToPixelIndex
  (geo::WireID const& wireid) const -> PixelIndex_t
{
  return wireToPixelIndex(wireid.Wire);
} // geo::PixelPlaneGeoInterface::wireToPixelIndex(WireID)
...
} // geo::PixelPlaneGeoInterface::wireToPixelIndex(WireLocator)
  ...
  (geo::WireID::WireID_t const wire) const -> PixelIndex_t
{
  ...
  return index;
} // geo::PixelPlaneGeoInterface::wireToPixelIndex()


//------------------------------------------------------------------------------
inline geo::WireGeo geo::PixelPlaneGeoInterface::getWire
  (WireLocator const& wloc) const
{
...
//------------------------------------------------------------------------------
template <typename Stream>
void geo::PixelPlaneGeoInterface::PrintPixelPlaneInfo(
  Stream&& out,
  std::string indent /* = "" */,
...
} // geo::PixelPlaneGeoInterface::PrintPixelPlaneInfo()


//------------------------------------------------------------------------------
template <typename Stream>
void geo::PixelPlaneGeoInterface::PrintPixelInfo(
  Stream&& out, PixelCoordID_t const coords,
  std::string indent /* = "" */, unsigned int verbosity /* = 1U */
  ) const
{
...
} // geo::PixelPlaneGeoInterface::PrintPixelInfo()


//------------------------------------------------------------------------------

#endif // LARCOREALG_GEOMETRY_PIXELPLANE_PIXELPLANEGEOINTERFACE_H
