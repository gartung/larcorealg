////////////////////////////////////////////////////////////////////////
/// \file  larcorealg/Geometry/SenseWireGeo.h
...
#ifndef LARCOREALG_GEOMETRY_SENSEWIREGEO_H
#define LARCOREALG_GEOMETRY_SENSEWIREGEO_H
...
#include "larcorealg/CoreUtils/UncopiableAndUnmovableClass.h" // geo::vect
...
namespace geo {
   ...
   *
   * @note This used to be `geo::WireGeo`, before it was conquered by
   *       `geo::PlaneGeo`. Now it is a refugee in `geo::WirePlaneGeo`.
   */
  class SenseWireGeo: private lar::UncopiableClass {

    using DefaultVector_t = geo::WireGeo::DefaultVector_t;
    using DefaultPoint_t = geo::WireGeo::DefaultPoint_t;
  ...
  public:
     ...
     * instances of `geo::SenseWireGeo` have the same type but are not compatible.
     */

    /// Tag for vectors in the "local" GDML coordinate frame of the plane.
    using WireGeoCoordinatesTag = geo::WireGeo::WireGeoCoordinatesTag;

    /// Type of points in the local GDML wire plane frame.
    using LocalPoint_t = geo::WireGeo::LocalPoint_t;

    /// Type of displacement vectors in the local GDML wire plane frame.
    using LocalVector_t = geo::WireGeo::LocalVector_t;
     ...
     * in the `SenseWireGeo` object.
     */
    SenseWireGeo(TGeoNode const& node, geo::TransformationMatrix&& trans);
    ...
    /// Returns if this wire is parallel to another
    bool isParallelTo(geo::SenseWireGeo const& wire) const
      {
        return // parallel if the dot product of the directions is about +/- 1
          std::abs(std::abs(Direction<geo::Vector_t>().Dot(wire.Direction<geo::Vector_t>())) - 1.) < 1e-5;
      }
     ...
     * `geo::SenseWireGeo::LocalPoint_t` and `geo::SenseWireGeo::LocalVector_t`,
     * respectively.
     */
    ...
    /// Returns the transformation object bridging local and world coordinates.
    auto const& GetTransformation() const { return fTrans; }
    ...
    double DistanceFrom(geo::SenseWireGeo const& wire) const;
    // @}
    ...
    static double WirePitch(geo::SenseWireGeo const& w1, geo::SenseWireGeo const& w2)
  ...
  static_assert(!std::is_copy_assignable_v<geo::SenseWireGeo>);
  static_assert(!std::is_copy_constructible_v<geo::SenseWireGeo>);
  static_assert( std::is_move_assignable_v<geo::SenseWireGeo>);
  static_assert( std::is_move_constructible_v<geo::SenseWireGeo>);
...
//------------------------------------------------------------------------------
template <typename Point>
Point geo::SenseWireGeo::GetPositionFromCenterUnbounded(double localz) const {
  return geo::vect::convertTo<Point>
    (toWorldCoords(LocalPoint_t{ 0.0, 0.0, relLength(localz) }));
} // geo::SenseWireGeo::GetPositionFromCenterImpl()


//------------------------------------------------------------------------------
template <typename Vector>
Vector geo::SenseWireGeo::Direction() const {
...
} // geo::SenseWireGeo::Direction()


//------------------------------------------------------------------------------
template <typename Stream>
void geo::SenseWireGeo::PrintWireInfo(
  Stream&& out,
  std::string indent /* = "" */,
...
//  if (verbosity-- <= 0) return; // 4
  ...
  //----------------------------------------------------------------------------
} // geo::SenseWireGeo::PrintWireInfo()


//------------------------------------------------------------------------------

#endif // LARCOREALG_GEOMETRY_SENSEWIREGEO_H
